...
class SphinxClient
{
	...
	/// set ranking mode
	public function set_ranking_mode ( $ranker, $rankexpr="" )
	{
	...
	public function add_query ( $query, $index="*", $comment="" )
	{
		...
		foreach ( $this->_filters as $filter )
		{
			...
			switch ( $filter["type"] )
			{
				...
				case SPH_FILTER_FLOATRANGE:
					$req .= $this->_pack_float ( $filter["min"] ) . $this->_pack_float ( $filter["max"] );
					break;
		...
		} else
		{
			...
			$req .= $this->_pack_float ( $a["lat"] ) . $this->_pack_float ( $a["long"] );
		}
		...
		foreach ( $this->_overrides as $key => $entry )
		{
			...
			foreach ( $entry["values"] as $id=>$val )
			{
				...
				switch ( $entry["type"] )
				{
					case SPH_ATTR_FLOAT:	$req .= $this->_pack_float ( $val ); break;
	...
	public function run_queries ()
	{
		...
		if ( !( $this->_send ( $fp, $req, $len+8 ) ) ||
			 !( $response = $this->_get_response ( $fp, VER_COMMAND_SEARCH ) ) )
		{
			$this->_mb_pop ();
			return false;
		}
	...
	public function build_excerpts ( $docs, $index, $words, $opts=array() )
	{
		...
		if ( !( $this->_send ( $fp, $req, $len+8 ) ) ||
			 !( $response = $this->_get_response ( $fp, VER_COMMAND_EXCERPT ) ) )
		{
			$this->_mb_pop ();
			return false;
		}
	...
	public function build_keywords ( $query, $index, $hits )
	{
		...
		if ( !( $this->_send ( $fp, $req, $len+8 ) ) ||
			 !( $response = $this->_get_response ( $fp, VER_COMMAND_KEYWORDS ) ) )
		{
			$this->_mb_pop ();
			return false;
		}
	...
	public function update_attributes ( $index, $attrs, $values, $mva=false )
	{
		...
		if (!( $response = $this->_get_response ( $fp, VER_COMMAND_UPDATE ) ))
		{
			$this->_mb_pop ();
			return -1;
		}
	...
	public function status ()
	{
		...
		if ( !( $this->_send ( $fp, $req, 12 ) ) ||
			 !( $response = $this->_get_response ( $fp, VER_COMMAND_STATUS ) ) )
		{
			$this->_mb_pop ();
			return false;
		}
	...
	public function flush_attributes ()
	{
		...
		if ( !( $this->_send ( $fp, $req, 8 ) ) ||
			 !( $response = $this->_get_response ( $fp, VER_COMMAND_FLUSHATTRS ) ) )
		{
			$this->_mb_pop ();
			return -1;
		}
